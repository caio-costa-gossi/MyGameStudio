...
CameraInstance::CameraInstance(const enums::CameraType cameraType, const enums::CameraControlScheme controlScheme, const float nearPlane, const float farPlane, const float perspectiveWidth, const float perspectiveHeight, const float perspectiveFovDegrees, const float orthoLeftPlane, const float orthoRightPlane, const float orthoTopPlane, const float orthoBottomPlane)
{
	assert(CameraManager::IsInit());

	if (cameraType == enums::orthogonal)
		CameraManager::CreateOrtho(globalCameraId_, controlScheme, orthoLeftPlane, orthoRightPlane, orthoTopPlane, orthoBottomPlane, nearPlane, farPlane);
	else
		CameraManager::CreatePerspective(globalCameraId_, controlScheme, perspectiveWidth, perspectiveHeight, perspectiveFovDegrees, nearPlane, farPlane);
}
